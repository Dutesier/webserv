...
TEST_F(test_config, TestWithFileDefault) {
    ...
    if (!MyFile.good())
        std::cout << "[ATTENTION HERE] Could not create file for testing, "
    ...
    EXPECT_FALSE(NULL == config) << "[ATTENTION HERE] Make sure you have a "
                                    "/tmp/webserv directory on your machine";
    remove("/tmp/webserv/webserv.config");
}

TEST_F(test_config, TestWithFileProvided) {
    int   argc = 2;
    char* argv[2] = {"webserv", "local.config"};

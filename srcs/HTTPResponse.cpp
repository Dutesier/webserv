...
std::string HTTPResponse::to_str(void) const {
	...
	std::string start_line = std::string( this->version + SP + code + SP + this->status.second );
	return (std::string(start_line /* + CRLF + header.to_str() */ + (body.empty() ? (CRLF + CRLF) : (CRLF + CRLF + body))));
}

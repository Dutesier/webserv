...
// // A la golang, parsing functions return a pair of the actual result and a
// bool
// // of success
// std::pair<webserv::Method, bool> HTTPParser::getMethod(std::string&
// firstLine) {
//     if (firstLine.empty()) { return std::make_pair(webserv::UNDEFINED,
//     false); }
...
// // // SYSCAL rcv triggers handler -> handler will keep calling
// getNextRequest()
...
// //
// //     // Declare variables
// //     char            buff[READING_BUFFER + 1]; // Buffer for reading data
// from
// //     socket static char     restOfData[READING_BUFFER + 1]; // Buffer for
// //     storing data left over from previous call to recv static bool
// //     dataInBuffer = false; // Flag to indicate whether there is data left
// over
// //     in the restOfData buffer size_t          bytes_read = 0; // Number of
// //     bytes read from the socket
// //
// //     char*       endOfHeaders = "\r\n\r\n"; // String to identify the end
// of
// //     the headers in the request size_t      eoh_position; // Position of
// the
// //     end of the headers in the buffer
// //
// //     // String to store the request headers
// //     std::string header;
// //     header.reserve(READING_BUFFER + 1); // Pre-allocate memory for the
// string
...
// //     // Get headers from data
// //     header = buff;
// //     if ((eoh_position = header.find(endOfHeaders, 0)) ==
// std::string::npos) {
// //         LOG_I("More than 8k header size");
...
// //             static_cast<int>(strlen(buff) - (eoh_position))); if
// (body_size <
...
// //
// //                 while (body_size > request->getContent().size()) { //
// While
...
// //                     }
// //                     temp = buff;
// //                     // What I want to do here is add what's missing until
// we
// //                     are of size: body_size int chars_still_missing =
// //                     body_size - (request->getContent().length()); // maybe
// +1
// //                     for the NTC? request->setContent(request->getContent()
// +
// //                     temp.substr(0,
// std::min(static_cast<int>(temp.length()),
// //                     chars_still_missing)));
// //                 }
// //             }
// //         }
// //
// //         // Clear what we have read from buffer (and store the rest in
// buff)
...
// //         *writer = formatted.c_str(); int index; for (index = 0;
// writer[index]
// //         != '\0'; ++index) {

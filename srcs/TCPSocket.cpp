...
bool TCPSocket::listen(void) { return (!::listen(this->fd, BACKLOG)); }

bool TCPSocket::shutdown(int how) { return (!::shutdown(this->fd, how)); }
...
std::string TCPSocket::recv(SocketConnection* connection) {
    ...
    if (connection == NULL) {
        ...
        else return ("No connection detected");
    }
...
bool TCPSocket::send(SocketConnection* connection, std::string response) {
    ...
    if (connection == NULL) {
        if (connections.size() > 0) connection = connections.at(0);
        else return ("No connection detected"); // or false maybe?
    }

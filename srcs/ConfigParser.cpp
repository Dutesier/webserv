...
bool ConfigParser::valid_end(std::vector<std::string>* commands) const {
    ...
    if (commands->back() == ";") commands->pop_back();
...
bool ConfigParser::listen_handler(std::vector<std::string> commands) {
    ...
    } else if (!this->is_port(commands[1])) address = commands[1];
    else port = commands[1];
...
bool ConfigParser::root_handler(std::vector<std::string> commands) {
    ...
    if (commands.empty() || commands[0] != "root") return (false);
    // checking if command size is valid
    if (commands.size() < 2) return (false);
    // checking if command ends with ';'
    if (!this->valid_end(&commands)) return (false);
...
bool ConfigParser::server_handler(std::vector<std::string> commands) {
    ...
    if (commands[0] == "server{" ||
        (commands.size() > 1 && commands[0] == "server" &&
         commands[1] == "{")) {
        if (this->config->server) return (false);
...
bool ConfigParser::end_block_handler(std::vector<std::string> commands) {
    ...
    // checking if command is empty
    // or if commands starts with listen
    if (commands.empty() || commands[0] != "}") return (false);
    // checking if command size is valid
    if (commands.size() != 1) return (false);

    if (this->config->location) this->config->location = false;
    else if (this->config->server) this->config->server = false;
    else return (false);
    return (true);
}

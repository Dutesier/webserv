...
public:
    ...
    virtual smt::shared_ptr<IHTTPValidator> setNext(smt::shared_ptr<IHTTPValidator> validator) = 0;
};
...
public:
    ...
    smt::shared_ptr<IHTTPValidator> setNext(smt::shared_ptr<IHTTPValidator> validator) {
        this->m_next = validator;
        return validator; // So that we can link validators: setNext(hasMethod)->setNext(hasVersion) etc.
    }

    ok_err isValid(smt::shared_ptr<HTTPRequest>& req) {
        if (this->m_next) {
            return this->m_next->isValid(req);
        }
        return {};
    }
